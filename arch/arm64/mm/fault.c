...
{
	// ! TEST
	int (*uca_fault_handler)(unsigned long, struct pt_regs *);
...
no_context:
	uca_fault_handler = (void *) kallsyms_lookup_name("uca_fault_handler");
	if (uca_fault_handler) {
		if (uca_fault_handler(addr, regs) == 1) {
			printk(KERN_ERR "DEBUG: PAGE FAULT\n");
		}
	}
	__do_kernel_fault(addr, esr, regs);
	return 0;
